# IED Navigator - Build Unattended ZIP Installer
# Developed by Emilio Medina
# Creates a self-contained ZIP with INSTALAR.bat for unattended setup
...
$OutputZip = "$ProjectRoot\$OutputName.zip"
...
if (Test-Path $npcapSrc) {
    ...
    Write-Host "  AVISO: Npcap no encontrado, el usuario debera instalarlo manualmente" -ForegroundColor Yellow
}
...
if "!NPCAP_FOUND!"=="1" (
    ...
    if exist "%~dp0npcap-1.79.exe" (
        ...
        echo   [!!] Npcap no encontrado y no hay instalador incluido.
...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%SHORTCUT%'); $s.TargetPath = '%~dp0IEDNavigator.bat'; $s.WorkingDirectory = '%~dp0'; $s.Description = 'IED Navigator - IEC 61850 Explorer'; $s.Save()" 2>nul
...
3. El instalador verificara e instalara automaticamente:
   - Java JDK 21 (si no esta instalado)
   - Npcap (si no esta instalado y se incluye el instalador)
...
- Npcap (se instala automaticamente para captura GOOSE/SV)
- Ejecutar como Administrador para captura de red
...
MODO IED-TO-IED:
-----------------
Para simular comunicacion entre dos IEDs:
